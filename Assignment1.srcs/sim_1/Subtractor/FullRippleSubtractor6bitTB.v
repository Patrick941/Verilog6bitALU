...
module FullRippleSubtractor6bitTB;
    // signal declaration
    reg  [5:0] vector0, vector1;
    wire [5:0] sumResult; 
    wire carryResult, overflow;

    FullRippleSubtractor6bit subtractor_unit (  .inputA(vector0), .inputB(vector1), .sumOutput(sumResult), .carryOut(carryResult), .overflow(overflow));
    initial
    begin
